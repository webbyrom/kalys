...
class RevAddOn404Update {
	private $plugin_url		 = 'https://codecanyon.net/item/slider-revolution-responsive-wordpress-plugin/2751380';
	private $remote_url_info = 'addons/revslider-404-addon/revslider-404-addon.php';
	private $plugin_slug	 = 'revslider-404-addon';
	private $plugin_path	 = 'revslider-404-addon/revslider-404-addon.php';
	private $version;
	private $plugins;
	private $option;
	...
	protected function _check_updates($force_check = false){
		...
		$last_check = get_option('revslider_404_addon-update-check');

		if($last_check == false){ //first time called
			$last_check = time();
			update_option('revslider_404_addon-update-check', $last_check);
		}
		
		//Check for updates
		if(time() - $last_check > '2592000‬' || $force_check == true){
			$data = $this->_retrieve_update_info();	

			if(isset($data->basic)){
				update_option('revslider_404_addon-update-check', time());
				
				$this->data->checked = time();
				$this->data->basic = $data->basic;
				$this->data->full = $data->full;
				
				update_option('revslider_404_addon-latest-version', $data->full->version);
			}
		}
	...
	public function _retrieve_update_info(){
		...
		$rattr = array(
			...
			'version' => urlencode(REV_ADDON_404_VERSION)
		);
