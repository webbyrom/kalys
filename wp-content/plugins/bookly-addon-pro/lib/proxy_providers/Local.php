    ...
    public static function createBackendPayment( array $data, CustomerAppointment $ca )
    {
        if ( isset( $data['payment_action'], $data['payment_for'] ) && $data['payment_action'] === 'create' && $data['payment_for'] === 'current' ) {
            ...
            if ( $ca->getExtras() != '[]' ) {
                ...
                foreach ( BooklyLib\Proxy\ServiceExtras::findByIds( array_keys( $_extras ) ) ?: array() as $extra ) {
            ...
            if ( Config::needCreateWCOrder( $total ) ) {
                $wc_order = Lib\Utils\Common::createWCOrder( $appointment->getServiceId(), $price, $tax, $customer->getWpUserId() );
                $payment_details['gateway_ref_id'] = $wc_order->get_id();
            ...
            $payment
                ->setTotal( $total )
                ->setTax( $tax )
                ->setDetails( json_encode( $payment_details ) )
                ->save();
