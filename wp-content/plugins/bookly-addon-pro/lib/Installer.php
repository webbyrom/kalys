...
class Installer extends Base\Installer
{
    ...
    public function __construct()
    {
        ...
        $this->options = array(
            ...
            'bookly_app_show_appointment_qr' => '0',
            'bookly_app_show_time_zone_switcher' => '0',
            'bookly_app_show_tips' => '0',
            'bookly_auto_change_status' => '0',
            ...
            'bookly_l10n_label_pay_cloud_square' => __( 'I will pay now with Credit Card', 'bookly' ),
            'bookly_l10n_label_cloud_gift' => __( 'Gift card', 'bookly' ),
            'bookly_l10n_label_pay_cloud_gift' => __( 'I will pay by gift card', 'bookly' ),
            // Settings -> URL.
            ...
            // Gift
            'bookly_cloud_gift_default_code_mask' => 'GIFT-****',
            'bookly_cloud_gift_enabled' => '0',
            'bookly_l10n_cloud_gift_error_expired' => __( 'This gift card has expired', 'bookly' ),
            'bookly_l10n_cloud_gift_error_invalid' => __( 'This gift card cannot be used for the current order', 'bookly' ),
            'bookly_l10n_cloud_gift_error_low_balance' => __( 'Gift card balance is not enough', 'bookly' ),
            'bookly_l10n_cloud_gift_error_not_found' => __( 'Gift card not found', 'bookly' ),
        );
    }
    ...
    public function createTables()
    {
        ...
        $wpdb->query(
            'CREATE TABLE IF NOT EXISTS `' . Entities\GiftCard::getTableName() . '` (
                `id` INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
        ...
        $wpdb->query(
            'ALTER TABLE `' . BooklyLib\Entities\Payment::getTableName() . '`
             ADD CONSTRAINT
                ...
                REFERENCES ' . Entities\GiftCard::getTableName() . '(id)
                ON DELETE SET NULL
                ON UPDATE CASCADE'
        );

        $wpdb->query(
            'CREATE TABLE IF NOT EXISTS `' . Entities\GiftCardService::getTableName() . '` (
                `id` INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
        ...
        $wpdb->query(
            'CREATE TABLE IF NOT EXISTS `' . Entities\GiftCardStaff::getTableName() . '` (
                `id` INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
                `gift_card_id` INT UNSIGNED NOT NULL,
                `staff_id` INT UNSIGNED NOT NULL,
                CONSTRAINT
