...
class Shared extends Proxy\Shared
{
    ...
    public static function preparePaymentOptions( $options, $form_id, $show_price, BooklyLib\CartInfo $cart_info, $userData )
    {
                ...
                $options[ BooklyLib\Entities\Payment::TYPE_CLOUD_SQUARE ] = array(
                    'html' => self::renderTemplate(
                        ...
                        array(
                            ...
                            'show_price' => BooklyLib\Proxy\Shared::showPaymentSpecificPrices( false ),
                            'cart_info' => $cart_info,
                            'payment_status' => $userData->extractPaymentStatus( $cart_info->getGateway() ),
                        ),
                        false
        ...
        if ( Proxy\CustomerGroups::allowedGateway( BooklyLib\Entities\Payment::TYPE_CLOUD_GIFT, $userData ) !== false ) {
            $pay_cloud_gift = BooklyLib\Cloud\API::getInstance()->account->productActive( BooklyLib\Cloud\Account::PRODUCT_GIFT ) && get_option( 'bookly_cloud_gift_enabled' );
            if ( $pay_cloud_gift ) {
                $cart_info->setGateway( BooklyLib\Entities\Payment::TYPE_CLOUD_GIFT );
                $options[ BooklyLib\Entities\Payment::TYPE_CLOUD_GIFT ] = array(
                    'html' => self::renderTemplate(
                        'cloud_gift_option',
                        array(
                            'form_id' => self::parameter( 'form_id' ),
        ...
        if ( Config::paypalEnabled() ) {
            switch ( get_option( 'bookly_paypal_enabled' ) ) {
                case Lib\Payment\PayPal::TYPE_EXPRESS_CHECKOUT:
                    Gateway::renderForm( $form_id, BooklyLib\Entities\Payment::TYPE_PAYPAL, $page_url, 'paypal-express-checkout' );
                    break;
                ...
                case Lib\Payment\PayPal::TYPE_CHECKOUT:
                    Gateway::renderForm( $form_id, BooklyLib\Entities\Payment::TYPE_PAYPAL, $page_url );
                    break;
            }
        }
        if ( get_option( 'bookly_cloud_square_enabled' ) && BooklyLib\Cloud\API::getInstance()->account->productActive( BooklyLib\Cloud\Account::PRODUCT_SQUARE ) ) {
           Gateway::renderForm( $form_id, BooklyLib\Entities\Payment::TYPE_CLOUD_SQUARE, $page_url );
        }
        if ( get_option( 'bookly_cloud_gift_enabled' ) && BooklyLib\Cloud\API::getInstance()->account->productActive( BooklyLib\Cloud\Account::PRODUCT_GIFT ) ) {
           Gateway::renderForm( $form_id, BooklyLib\Entities\Payment::TYPE_CLOUD_GIFT, $page_url );
        }
    }
