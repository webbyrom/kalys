    ...
    $(document.body).on('bookly.init_email_logs', {},
        function(event) {
            ...
            $.each(BooklyEmailLogsL10n.datatables.email_logs.settings.order, function(_, value) {
                const index = dt_columns.findIndex(function(c) { return c.data === value.column; });
                if (index !== -1) {
            ...
            if (dt_columns.length) {
                dt = $table.DataTable({
                    ...
                    ajax: {
                        ...
                        type: 'POST',
                        data: function(d) {
                            return $.extend({action: 'bookly_pro_get_email_logs', csrf_token: BooklyL10nGlobal.csrf_token}, {
                                filter: {
            ...
            $search.on('keyup', function() { dt.ajax.reload(); });
        }
    );
});
