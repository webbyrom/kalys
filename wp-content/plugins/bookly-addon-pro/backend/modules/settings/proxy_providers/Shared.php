...
class Shared extends Proxy\Shared
{
    ...
    {
        $payment_data[ BooklyLib\Entities\Payment::TYPE_PAYPAL ] = self::renderTemplate( 'paypal_settings', array(), false );
        if ( BooklyLib\Cloud\API::getInstance()->account->productActive( BooklyLib\Cloud\Account::PRODUCT_SQUARE ) ) {
            $payment_data[ BooklyLib\Entities\Payment::TYPE_CLOUD_SQUARE ] = self::renderTemplate( 'cloud_square_settings', array(), false );
        }
        if ( BooklyLib\Cloud\API::getInstance()->account->productActive( BooklyLib\Cloud\Account::PRODUCT_GIFT ) ) {
            $payment_data[ BooklyLib\Entities\Payment::TYPE_CLOUD_GIFT ] = self::renderTemplate( 'cloud_gift_settings', array(), false );
        }
    ...
    public static function saveSettings( array $alert, $tab, array $params )
    {
        ...
        switch ( $tab ) {
            ...
            case 'payments':
                ...
                if ( BooklyLib\Cloud\API::getInstance()->account->productActive( BooklyLib\Cloud\Account::PRODUCT_GIFT ) ) {
                    $options[] = 'bookly_cloud_gift_enabled';
                }
                break;
            case 'purchase_code':
